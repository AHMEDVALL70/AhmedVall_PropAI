/**
 * AhmedVall PropAI — الخادم الخلفي الآمن (Cloudflare Worker)
 * -----------------------------------------------------------
 ...
 * 3) سجّل الدخول:
 *      wrangler login
 * 4) في مجلد فيه هذا الملف وحده، أنشئ ملف wrangler.toml بهذا المحتوى:
 *
 ...
 *      https://ahmedvall-propai-backend.YOUR-SUBDOMAIN.workers.dev
 * 8) افتح index.html وضع هذا الرابط + "/chat" في متغيّر BACKEND_URL أعلى قسم السكربت.
 *
 * ملاحظة أمان: ALLOWED_ORIGIN أدناه يقيّد من يستطيع استخدام الخادم بموقعك فقط.
 * غيّره إلى دومين موقعك الفعلي بعد النشر (مثال: https://ahmedvall70.github.io) بدل "*".
 */

const ALLOWED_ORIGIN = "*"; // ⚠️ غيّرها إلى دومين موقعك تحديداً بعد التجربة، مثل: "https://ahmedvall70.github.io"
...
export default {
  async fetch(request, env) {
    // طلبات التحقق المسبق من المتصفح (CORS preflight)
    ...
    // حد أساسي بسيط لطول المحادثة لمنع إساءة استخدام الخادم (تقدر تشدّده لاحقاً)
    ...
    try {
      const anthropicResponse = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        headers: {
          ...
          "x-api-key": env.ANTHROPIC_API_KEY, // المفتاح موجود فقط هنا، على الخادم
